...
import numpy as np
from data import dataset_queue_prefetch
...
def validate_run(sess, images, labels, is_training_tensor,
    ...
    for j in range(val_step_num):
        ...
        loss_val, acc_val = sess.run([total_loss, acc],
                                     feed_dict={images: val_images_batch, labels: val_labels_batch, is_training_tensor:False})
...
def main(_):
    ...
    train_images, train_labels =  dataset_queue_prefetch.inputs("train", batch_size=FLAGS.batch_size,
                                                    dataset_dir = FLAGS.data_dir, num_epochs=FLAGS.epoch_num,
                                                    is_training=True)

    val_images, val_labels =  dataset_queue_prefetch.inputs("validation", batch_size=FLAGS.batch_size,
                                        dataset_dir = FLAGS.data_dir, num_epochs=FLAGS.epoch_num,
                                        is_training=False)
