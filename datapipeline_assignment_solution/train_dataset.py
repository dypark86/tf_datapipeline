...
import numpy as np
from data import dataset_queue
...
def validate_run(sess, images, labels, is_training_tensor,
                 val_step_num, val_next_element,
                 total_loss, acc):
    val_loss_list = []
    val_acc_list = []
    for j in range(val_step_num):
        val_images_batch, val_labels_batch = sess.run(val_next_element)
...
def main(_):
    ...
    train_dataset =  dataset_queue.inputs("train", batch_size=FLAGS.batch_size,
                                                    dataset_dir = FLAGS.data_dir, num_epochs=FLAGS.epoch_num,
                                                    is_training=True)

    val_dataset =  dataset_queue.inputs("validation", batch_size=FLAGS.batch_size,
                                        dataset_dir = FLAGS.data_dir, num_epochs=FLAGS.epoch_num,
                                        is_training=False)
    ...
    with tf.Session() as sess:
        ...
        while True:
            try:
                ...
                train_images_batch, train_labels_batch = sess.run(train_next_element)
